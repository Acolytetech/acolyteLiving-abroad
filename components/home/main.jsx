...
export default function Main() {
  return (
    <main className="bg-[#f8fafc] text-slate-900 font-sans selection:bg-[#161D83] selection:text-white pb-10 pt-20">
      ...
      <section className="relative text-center py-10 md:py-24 px-4 max-w-7xl mx-auto">
        ...
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-blue-100/60 border border-slate-200 text-[11px] font-bold uppercase tracking-widest mb-6 text-[#161D83] shadow-sm">
          <ShieldCheck className="w-6 h-6 text-[#161D83]" />
          <span>0% Commission · No Agents · AI Powered · All in one place</span>
        </div>

        <h1 className="text-4xl md:text-7xl font-extrabold leading-[1.1] tracking-tight text-slate-900 mb-6">
        ...
        <p className="text-lg md:text-xl text-slate-500 max-w-2xl mx-auto mb-10 leading-relaxed font-medium">
          Upload your resume once — let our AI match you with the perfect colleges, jobs, and homes abroad in seconds.
        </p>
      ...
      <section className="max-w-5xl mx-auto px-4 pb-20">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            { n: "1000+", l: "Universities", icon: GraduationCap },
            { n: "50K+", l: "Jobs Listed Abroad", icon: Briefcase },
            { n: "25K+", l: "Verified Rooms", icon: Home },
          ].map((item, i) => (
            <div key={i} className="bg-white px-8 py-8 rounded-[24px] text-center shadow-xl shadow-slate-200/50 border border-slate-100 group hover:-translate-y-1 transition-transform duration-300">
              <div className="w-12 h-12 bg-blue-50 text-[#161D83] rounded-2xl flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-transform duration-300">
                <item.icon className="w-6 h-6" />
              </div>
              <h2 className="text-4xl font-semibold text-[#161D83] mb-1">{item.n}</h2>
              <p className="text-[13px] font-medium text-slate-400 uppercase tracking-widest">{item.l}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* UPLOAD */}
      <section className="max-w-5xl mx-auto text-center py-10 md:py-20 px-4">
        <div className="mb-12">
            <p className="text-[#161D83] font-semibold text-sm mb-6 uppercase">How it works</p>
        ...
        <div className="bg-white border-2 border-dashed border-blue-200 bg-gradient-to-b from-blue-50/50 to-transparent p-12 rounded-[32px] max-w-2xl mx-auto hover:border-[#161D83]/50 hover:bg-blue-50/80 transition-all duration-300 group cursor-pointer shadow-lg shadow-blue-100/50">
          ...
          <p className="text-[15px] font-medium text-slate-500 mb-8">
            Supports PDF or DOCX up to 5MB. We handle the rest.
          </p>
  ...
  <div className="flex flex-row justify-center items-center gap-4 mt-12 relative z-10">
          {["Upload resume", "AI finds matches", "Apply", "Move abroad"].map((s, i) => (
  ...
  {["Upload resume", "AI finds matches", "Apply", "Move abroad"].map((s, i) => (
    <div key={i} className="flex items-center gap-3">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {[
            ...
            <div key={i} className="bg-white overflow-hidden rounded-[32px] shadow-xl shadow-slate-200/40 border border-slate-100 flex flex-col hover:-translate-y-2 transition-transform duration-300">
              ...
              <div className="w-14 h-14 bg-blue-50 text-[#161D83] rounded-2xl flex items-center justify-center mb-4">
                
                <card.icon className="w-7 h-7" />
              </div>
              <h3 className="font-bold text-2xl text-slate-900 mb-4">{card.title}</h3>
              <p>{card.desc}</p>
              </div>
              <ul className="space-y-4 mb-10 flex-1 px-4 pt-6">
                {card.points.map((p, idx) => (
                  <li key={idx} className="flex items-start gap-3 text-[15px] font-normal text-slate-600">
      ...
      <section className="max-w-5xl mx-auto px-6 py-10">
        <h2 className="text-3xl font-bold text-center text-slate-900 mb-12">Everything you need, in one place.</h2>
      ...
      <section className="max-w-4xl mx-auto text-center py-10 md:py-20 px-4">
        <p className="uppercase text-[#161d83] font-medium">destinations</p>
        <div className="inline-flex items-center justify-center gap-2 mb-8">
          <MapPin className="w-6 h-6 text-[#161D83]" />
          <h2 className="text-2xl md:text-3xl font-bold text-slate-900">Where do you want to go?</h2>
        </div>
        ...
        <button className=" flex items-center mx-auto  bg-transparent text-[#161D83] px-6 py-10 font-medium text-[15px] tracking-wide hover:bg-[#161D83] hover:text-white transition-colors active:scale-95">
       Help me choose a country <ArrowRight className="w-4 h-4" />
        </button>
      </section>
...
{/* white color ellips overplay 200px   */}
          ...
          <div className="relative z-10 max-w-3xl mx-auto">
            ...
            <p className="text-blue-100 text-lg mb-10">Join thousands of students and professionals who found their dream opportunities with Acolyte.</p>
            <button className="bg-white text-[#161D83] hover:bg-blue-50 px-8 py-4 rounded-xl font-medium text-[15px] tracking-wide transition-colors shadow-xl flex items-center justify-center gap-2 mx-auto active:scale-95">
